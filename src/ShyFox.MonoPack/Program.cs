...
const string LINUX_ARM64_RID = "linux-arm64";// runtime identifier for 64-bit linux build.
...
try
{
    ...
    if (_runtimeIdentifiers.Contains(LINUX_ARM64_RID))
    {
        BuildLinuxArm();
        PackageLinux(LINUX_ARM64_RID);
    }
...
void BuildLinuxArm()
{
    string outputDir = Path.Combine(_outputDir, LINUX_ARM64_RID);
    Build(outputDir, LINUX_ARM64_RID);
}
...
void PackageLinux(string rid = LINUX_RID)
{
    string projectName = Path.GetFileNameWithoutExtension(_projectPath);
    string sourceDir = Path.Combine(_outputDir, rid);
    string tarPath = Path.Combine(_outputDir, $"{_executableFile ?? projectName}-{rid}.{(_useZip ? "zip" : "tar.gz")}");
...
void ValidateArguments()
{
    ...
    if (_runtimeIdentifiers.Count == 0)
    {
        ...
        else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
        {
            ...
            _runtimeIdentifiers.Add("linux-arm64");
        }
    }
...
void DisplayHelp()
{
    Console.WriteLine(
        ...
        Examples:
            ...
            monopack -p ./src/MyGame.csproj -o ./artifacts/builds -rids win-x64,osx-x64,osx-arm64,linux-x64,linux-arm64 -i ./Info.plist -c ./Icon.icns
        """
    );
}
